...
export default function UserCard({ user, compact = false, onClick }) {
  ...
  if (compact) {
    return (
      <button
        onClick={onClick}
        className="w-full flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-zinc-900 transition text-left"
      >
        {hasAvatar ? (
          <img
            src={user.avatar}
  ...
  return (
    <button
      onClick={onClick}
      className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-left hover:border-indigo-500/40 transition"
    >
